...
class ClipCriterion:
    ...
    def init_a_clip(self, batch: Dict, hidden_dim: int, num_classes: int, device: torch.device):
        ...
        if self.aux_loss:
            self.loss = {
                ...
                # 
                "box_ciou_RepGt" : torch.zeros(()).to(self.device),
                "box_ciou_RepPred" : torch.zeros(()).to(self.device)
            }
        else:
            self.loss = {
                "box_l1_loss": torch.zeros(()).to(self.device),
    ...
    def get_sum_loss_dict(self, loss_dict: dict):
        def get_weight(loss_name):
            ...
            elif "box_ciou_RepGt" in loss_name:
                return self.weight["box_ciou_RepGt"]
            elif "box_ciou_RepPred" in loss_name:
                return self.weight["box_ciou_RepPred"]
    ...
    def process_single_frame(self, model_outputs: dict, tracked_instances: List[TrackInstances], frame_idx: int):
        ...
        loss_l1, loss_giou = self.get_loss_box(outputs=model_outputs,
                                               gt_trackinstances=gt_trackinstances,
                                               idx_to_gts_idx=outputs_idx_to_gts_idx)
        # 9.+ Compute the bounding box Repulsion loss
        # 计算相邻GT的遮挡问题 CIOU(RepGT+RepBox)
        # 9.1 计算GT之间的遮挡情况
        
        loss_repulsion_gt, loss_repulsion_pred = self.get_repulsion_loss(outputs=model_outputs,
                                gt_trackinstances=gt_trackinstances,
                                idx_to_gts_idx=outputs_idx_to_gts_idx)
        ...
        self.loss["box_ciou_RepGt"] += loss_repulsion_gt * self.frame_weights[frame_idx]
        self.loss["box_ciou_RepPred"] += loss_repulsion_pred * self.frame_weights[frame_idx]
        # Update logs.
        ...
        self.log[f"frame{frame_idx}_box_ciou_RepGt"] = loss_repulsion_gt.item()
        self.log[f"frame{frame_idx}_box_ciou_RepPred"] = loss_repulsion_pred.item()
        ...
        if self.aux_loss:
            for i, aux_outputs in enumerate(model_outputs["aux_outputs"]):
                ...
                aux_loss_l1, aux_loss_giou = self.get_loss_box(outputs=model_outputs["aux_outputs"][i],
                                                               gt_trackinstances=gt_trackinstances,
                                                               idx_to_gts_idx=aux_idx_to_gts_idx)
                loss_repulsion_gt, loss_repulsion_pred = self.get_repulsion_loss(outputs=model_outputs["aux_outputs"][i],
                                gt_trackinstances=gt_trackinstances,
                                idx_to_gts_idx=aux_idx_to_gts_idx)
                ...
                self.loss["box_ciou_RepGt"] += loss_repulsion_gt * self.frame_weights[frame_idx] * self.aux_weights[i]
                self.loss["box_ciou_RepPred"] += loss_repulsion_pred * self.frame_weights[frame_idx] * self.aux_weights[i]
    ...
    def bbox_iou_(self, box1, box2, eps=1e-7):
        # (3200, 1, 4)
        box1 = box1[:, None]
        # (1, 3200, 4)
        box2 = box2[None]
        ...
        # 计算交集的宽度和高度
        inter_width = torch.min(b1_x2, b2_x2) - torch.max(b1_x1, b2_x1)
        inter_height = torch.min(b1_y2, b2_y2) - torch.max(b1_y1, b2_y1)

        # 计算交集面积和并集面积
        inter = torch.clamp(inter_width, min=0) * torch.clamp(inter_height, min=0)
    ...
    def get_repulsion_loss(self, outputs, gt_trackinstances: List[TrackInstances], idx_to_gts_idx):
        """
        Compute the bounding box loss, l1 and giou.
        """
        for b in range(len(gt_trackinstances)):
            # gt > pred
            if gt_trackinstances[b].boxes.shape[0] > idx_to_gts_idx[0][0].shape[0]:
                loss_repulsion_gt, loss_repulsion_pred = 0.0, 0.0
                continue
            matched_gt_boxes = [gt_trackinstances[b].boxes]
            matched_pred_boxes = [
                boxes[outputs_idx[0][outputs_idx[1] >= 0]]
                for boxes, outputs_idx in zip(outputs["pred_bboxes"], idx_to_gts_idx)
            ]
            cal_gt_tf = self.bbox_iou_(matched_gt_boxes[0], matched_gt_boxes[0]) > 0.5
            # 使用 torch.eye 函数创建对角线张量，并取反
            diagonal = ~torch.eye(matched_gt_boxes[0].size(0), dtype=torch.bool).to(device=matched_gt_boxes[0].device)
            cal_gt_tf = cal_gt_tf & diagonal
            loss_repulsion_gt = sum(self.bbox_iou_(matched_gt_boxes[0], matched_pred_boxes[0])[cal_gt_tf]) / 2.0
            loss_repulsion_pred = sum(self.bbox_iou_(matched_pred_boxes[0], matched_pred_boxes[0])[cal_gt_tf]) / 2.0
        if loss_repulsion_gt == 0.0 or loss_repulsion_pred == 0.0:
            return torch.as_tensor(loss_repulsion_gt, device=outputs["pred_bboxes"].device), torch.as_tensor(loss_repulsion_pred, device=outputs["pred_bboxes"].device)
        return loss_repulsion_gt, loss_repulsion_pred
...
def build(config: dict):
    ...
    return ClipCriterion(
        ...
        weight={
            ...
            "box_ciou_RepGt": config["LOSS_WEIGHT_REPGT"],
            "box_ciou_RepPred": config["LOSS_WEIGHT_REPPRED"]
        },
