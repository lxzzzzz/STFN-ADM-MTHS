...
class QueryUpdater(nn.Module):
    def __init__(self, hidden_dim: int, ffn_dim: int,
        ...
        self.memory_attn_shake = nn.MultiheadAttention(embed_dim=self.hidden_dim, num_heads=8, batch_first=True)
        self.memory_attn_mem = nn.MultiheadAttention(embed_dim=self.hidden_dim, num_heads=8, batch_first=True)
        ...
        # gate fusion -> batch最多包括4帧
        self.pre_tran = nn.Linear(256, 64)
        self.prepre_tran = nn.Linear(256, 64)
        self.fusion_conv = nn.Linear(128, 256)
    ...
    def update_tracks_embedding(self, tracks: List[TrackInstances]):
        for b in range(len(tracks)):
            if tracks[b].dif_track_ins.numel() == 0:
                continue
            # 1.dif_track_ins as Q and K       output_embed as V
            dif_q_k, dif_v = tracks[b].dif_track_ins, tracks[b].output_embed
            ...
            dif_q_k = dif_q_k + query_pos
            dif_v_ = self.memory_attn_shake(dif_q_k[None, :], dif_q_k[None, :], dif_v[None, :])[0][0, :]
            dif_vv = dif_v_ # as v
            # 3.遮挡处理 相邻帧输出通过gate fusion叠加 as Q  short memory
            dif_v_0 = dif_v_ + tracks[b].last_output
            dif_v_1 = dif_v + tracks[b].last_output
            
            dif_v_ = self.fusion_conv(torch.cat((self.act(self.pre_tran(dif_v_0)), self.act(self.prepre_tran(dif_v_1))), dim=-1))
            
            dif_v = dif_v + self.memory_dropout(dif_v_)
            dif_v = self.memory_norm(dif_v)
            # 此时 gate_fusion后的v作为q，抖动处理后的v作为v，long_memory作为k
            dif_q = self.memory_ffn(dif_v)
            # 5. use long memory and short memory
            long_memory = tracks[b].long_memory.detach()
            dif_k = long_memory + query_pos
            dif_vv = self.memory_attn_mem(dif_q[None, :], dif_k[None, :], dif_vv[None, :])[0][0, :]
            query_feat = long_memory + self.query_feat_dropout(dif_vv)
            query_feat = self.query_feat_norm(query_feat)
            query_feat = self.query_feat_ffn(query_feat)
            # 6. update long memory
            long_memory_lambda = self.confidence_weight_net(dif_v_)
            long_memory = (1 - long_memory_lambda) * long_memory + long_memory_lambda * dif_v_

            tracks[b].long_memory = tracks[b].long_memory * ~is_pos.reshape((is_pos.shape[0], 1)) + \
                                    long_memory * is_pos.reshape((is_pos.shape[0], 1))
            # 7.Update Last Outputs Embedding
            tracks[b].last_output = tracks[b].last_output * ~is_pos.reshape((is_pos.shape[0], 1)) + \
                                    dif_v_ * is_pos.reshape((is_pos.shape[0], 1))
            if self.use_dab:
                ...
                new_query_pos = self.linear_pos2(self.activation(self.linear_pos1(dif_v_)))
        ...
        if self.training:
            for b in range(len(new_tracks)):
                ...
                if self.tp_drop_ratio == 0.0 and self.fp_insert_ratio == 0.0:
                    ...
                    active_tracks.dif_track_ins = active_tracks.output_embed - active_tracks.last_output
